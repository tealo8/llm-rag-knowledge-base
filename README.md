...
| 范围 | 状态 | 证据与边界 |
| --- | --- | --- |
| 后端与权限回归 | 本机已验证 | `32 passed`；覆盖租户、知识库、仅上传、文档 ACL、版本、XLSX/CSV、分片上传、会话管理分享、反馈、死信权限、文件头和指标路径 |
...
- PDF、DOCX、TXT、Markdown、XLSX、CSV 上传；大于 8 MiB 时前端自动走顺序分片上传，服务端总上限可配置。
...
- 多轮会话、上下文截断、历史恢复、会话重命名/收藏/分支、带有效期和密码的只读分享、Markdown/PDF 导出、点赞/点踩原因。
- 问答页支持 Rerank、Top-K、快速/深度模式、模型/temperature/top-p 参数、全文和代码复制、引用详情弹窗与快捷追问。
- 引用文档、文件名、页码/段落、原文片段；点击引用后再次执行接口 ACL。
- 组织管理员、知识库管理员、普通成员；知识库级 `view/upload/edit/admin` 权限；知识库可独立配置 Prompt、Top-K、阈值、Rerank、问答和上传开关。
- 文档级全组织、私有、指定用户组或指定用户；列表、下载、引用与检索共用后端 ACL。
- 用户创建/停用、用户组、知识库授权、审计日志搜索/动作筛选/IP/结果字段、RAG 与安全参数管理。
- 上传处理阶段：队列、解析、向量化、失败原因和任务 ID；支持在线预览、标签筛选、批量重索引/删除；本地默认同步，Compose 默认 Celery + Redis 异步。
...
- `/api/conversations`：历史、重命名、收藏、分支、分享、反馈、Markdown/PDF 导出和删除
...
- 会话分享的 `continue` 目前只是授权模式字段，匿名页面仍按只读处理；开放继续问答前需要登录态、短期令牌和完整 ACL。PDF 导出支持中文与自动分页，但版式定位为审计留档，不替代正式报告模板。
